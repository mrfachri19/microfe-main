...
import { Earth16Filled, PersonLock16Filled, CaretRight24Regular, ChevronRightRegular, Send16Filled, LockClosed16Filled } from "@fluentui/react-icons";
...
// ** Reactstrap Imports
import { Badge, Card, CardHeader, Row, Col, CardTitle, CardBody, CardFooter, CardText, Button, Input, InputGroup } from 'reactstrap'


const CardTimeline = ({ dataId }) => {
    ...
    function postComment(id) {
        ...
        let html =
            `<div class="d-flex align-items-start mb-1" id="tobereplace" style="padding:8px;">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-4 col-md-4">
                            <div class="d-flex align-items-end justify-content-end">
                                <a class="mb-0" id="comment-delete-${id}"></a>
                            </div>
                        </div>
                    </div>
                        <small>${text}</small>
        ...
        if (text !== '') {
            commentPost(
                ...
                if (res.data.status === '200') {
                    ...
                    document.getElementById(`tobereplace`).setAttribute("id", `comment-${res.data.data.id}`);
                    let text = document.getElementById(`value-comment-post-${id}`).value = '';
                    document.getElementById(`comment-delete-${id}`).addEventListener("click", function () {
                        deleteComment(res.data.data.id, id)
                    }, false);
                    document.getElementById(`comment-delete-${id}`).innerHTML = 'x';
                    let newTotal = Number(document.getElementById(`comment-count-${id}`).innerText) + 1;
                    if (newTotal == 0) {
                        document.getElementById(`comment-count-${id}`).textContent = '';
                    } else {
                        document.getElementById(`comment-count-${id}`).textContent = newTotal;
                    }
                }
            })
        }
        ...
        loadCommentById(
            ...
            if (res.data.status === '200') {
                res.data.data.forEach(element => {
                    ...
                    html += `<div id="comment-${element.id}" class="d-flex align-items-start mb-1" style="padding:8px;">
                            <div class="avatar mt-25 me-75">
    ...
    function mapPrivacy(id) {
        if (id == 1) {
            ...
            return <LockClosed16Filled style={{ width: "24px", color: "#A0AEC0" }} height="100%" />
        } else if (id == 7) {
            return <Earth16Filled style={{ width: "24px", color: "#A0AEC0" }} height="100%" />
        }
    }
            ...
            return (
                <Card className='card-transaction bg-tertiary-100' key={`post-${item.id}`}>
                    <CardBody>
                                        ...
                                        <div className='profile-user-info'>
                                            <div>
                                                <Link to={`/profile/${item.owner.username}`} className="fw-bold">
                                                    <small>{item.owner.fullname}</small>
                                                </Link>
                                                &nbsp; ke &nbsp;
                                                <Link to={`/profile/${item.receiver.username}`} className="fw-bold">
                                                    <small>{item.receiver.fullname}</small>
                                                </Link>
                                            </div>
                                            <div>
                                                <small>{item.owner.jabatan}</small>
                                            </div>
                                            <div>
                                                <small className='text-muted'>{item.updatedAt}</small>
                                                <small className='text-muted'>{mapPrivacy(item.privacy)}</small>
                        ...
                        {item.videoPath && item.sharedSource == null ? (
                            <video controls>
                                <source src={item.videoPath.replace('_%d_', '_960_')} type="video/mp4" />
                        ...
                        <div className="post-comments" id={`comment-post-${item.id}`}>
                            {item.comments.map(comment => (
                                <div id={`comment-${comment.id}`} key={`comment-${comment.id}`} className='d-flex align-items-start mb-1' style={{ padding: '8px' }}>
                        ...
                        <div>
                            <fieldset className='form-label-group mb-50'>
                                <div className="mb-2">
                                    <div className="mb-2">
                                        <InputGroup>
                                            <Input placeholder="Tambahkan komentar" type="text" id={`value-comment-post-${item.id}`} className="form-control" />
                                            <Button onClick={(e => postComment(item.id))} className="px-4 py-2" color="primary">
                                                <Send16Filled />
                                            </Button>
                                        </InputGroup>
                                    </div>
